  ...
  head: () => ({
    meta: [
      ...
      { title: "EMD Inventory" },
      {
        name: "description",
        content:
          "Offline-first inventory, orders and customer management for the EMD sales team.",
      },
      { property: "og:title", content: "EMD Inventory" },
      {
...
function SyncWatcher() {
  useEffect(() => {
    const run = async () => {
      ...
      if (failed > 0) toast.error("Some offline changes could not be synced yet");
    };
    void run();
